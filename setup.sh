...
ALLOWED_IP=$(curl -sS "https://raw.githubusercontent.com/GeKaStore/izin/main/ip" | grep "$MYIP" | awk '{print $4}')
...
clear
    echo -e " ${UK}┌──────────────────────────────────────────┐${Suffix}"
    echo -e "   ${y}${f}            WUZZSTORE           ${NC}"
    echo -e " ${UK}└──────────────────────────────────────────┘${Suffix}"
    echo -e " ${UK}┌──────────────────────────────────────────┐${Suffix}"
    echo -e " ${UK}│$NC${WHITE}     Welcome To SPS Script Installer ${NC}"
    echo -e " ${UK}│$NC${WHITE} This Will Quick Setup VPN On Your Server${NC}"
    echo -e " ${UK}│$NC${WHITE}          © Recode By WUZZSTORE [ 2024 - 2025 ] ${NC}"
    echo -e " ${UK}└──────────────────────────────────────────┘${Suffix}"
}
Banner_Newbie
...
echo "WUZZSTORE" > /etc/xray/username
echo ""
clear
red='\e[1;31m'
green='\e[0;32m'
...
clear
rm -f /usr/bin/user
username=$(curl https://raw.githubusercontent.com/GeKaStore/izin/main/ip | grep $MYIP | awk '{print $2}')
echo "$username" >/usr/bin/user
expx=$(curl https://raw.githubusercontent.com/GeKaStore/izin/main/ip | grep $MYIP | awk '{print $3}')
echo "$expx" >/usr/local/sbin/e
...
Exp1=$(curl https://raw.githubusercontent.com/GeKaStore/izin/main/ip | grep $MYIP | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
sts="${Info}"
else
sts="${Error}"
...
clear
restart_system() {
USRSC=$(wget -qO- https://raw.githubusercontent.com/GeKaStore/izin/main/ip | grep $MYIP | awk '{print $2}')
EXPSC=$(wget -qO- https://raw.githubusercontent.com/GeKaStore/izin/main/ip | grep $MYIP | awk '{print $3}')
...
Pass    : 
────────────────────
  WUZZSTORE AUTOSCRIPT  
────────────────────
Automatic Notification from Github</code>
"'&reply_markup={"inline_keyboard":[[{"text":"🔥Tele","url":"https://t.me/WuzzSTORE"},{"text":"🔥WA","url":"https://wa.me/6287760204418"}]]}'
...
mkdir /root/.acme.sh
curl https://get.acme.sh | sh -s email=kalianber6@gmail.com
...
curl -sS ${REPO}install/password | openssl aes-256-cbc -d -a -pass pass:GeKaStore -pbkdf2 > /etc/pam.d/common-password
...
printf "q\n" | rclone config
curl -sS ${REPO}install/rclone.conf | openssl aes-256-cbc -d -a -pass pass:GeKaStore -pbkdf2 >/root/.config/rclone/rclone.conf 
cd /bin
...
fun_bar() {
    ...
    (
        ...
        # Jalankan perintah di background dan log output
